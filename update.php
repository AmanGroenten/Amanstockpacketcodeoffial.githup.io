...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_description)){
        $description_err = "Please enter the descriptiont.";     
    ...
    if(empty($name_err) && empty($stock_err) && empty($description_err)){
        ...
        $sql = "UPDATE stock SET name=?, stock=?, description=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_stock, $param_description, $param_id);
